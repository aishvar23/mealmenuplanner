 ...
 *
 * Dual-audience framing (ADO #86) mirrors the web two-panel sign-in: the same
 * Supabase auth serves households and meal providers. On native, post-login
 * routing is handled by the workspace switcher (there is no pre-auth `next`), so
 * the audience panels are informational — a provider signs in with the shared
 * form below, then opens (or sets up) their provider workspace from the chooser.
 */
...
/** One audience panel (households / meal providers) above the shared form. */
function AudiencePanel({
  ...
  return (
    <View
      ...
      className="flex-row gap-3 rounded-xl border border-gray-200 bg-gray-50 p-3"
    >
      ...
      >
        ...
        >
          ...
          <View className="mb-6 gap-3">
            <AudiencePanel
              icon={<Users color="#16a34a" size={18} />}
              title="For households"
              subtitle="Plan meals and answer “what should we eat today?”"
            />
            <AudiencePanel
              icon={<Store color="#16a34a" size={18} />}
              title="For meal providers"
              subtitle="Sign in, then open or set up your provider workspace."
            />
          </View>
