...
### MP-A-012E — Menu structural-edit guard (ADR-7 enforcement) — `DONE` (#22, migration `pmp_20_menu_edit_revision`)

- **Track:** A · **Checkpoint:** 3 · **Branch:** provider-services-api · **Conflict risk:** Low.
- **Objective:** Enforce the approved menu-edit-after-response policy (ADR-7 = REVISION) in the domain layer: detect whether any member response exists for a menu day and, when one does, route a structural edit (components/alternatives/customizations/cutoff) to a **new revision (rev N+1)** with response carry-forward + selective invalidation rather than mutating in place; a non-structural edit (note) applies in place. Surface `VALIDATION_ERROR`/`CONFLICT` with the appropriate `details.reason`. Consumed by MP-A-121 (edit RPC) and MP-B-030 (builder affordance). The **fresh-publish** writer (no responses) already shipped under MP-A-121.
...
- **Shipped (PR AB#22, `pmp_20`):** the guard is realised inside the `edit_provider_menu_day` writer (next entry) rather than a standalone domain helper — it locks the day, owner-gates, rejects a locked/archived/superseded/past-cutoff day (`MESTA` → `menu_not_editable` 409), and branches on `exists(response for day)`: no responses → in-place tree rebuild; responses → rev N+1. Non-structural note edits route to the separate `update_provider_menu_day_note` RPC (in place, any non-terminal day). `MECUT` → `cutoff_invalid` (a revision can't reopen a closed window). Verified by a rolled-back MCP probe (in-place same-id rebuild; revision carry-forward + selective per-component invalidation + re-confirm fan-out; MEOWN/MESTA/MECUT gates) + Vitest service tests.
- **Tests:** structural edit on a day with responses → revision (not silent in-place mutation); non-structural note edit applies in place; correct error reasons.
- **Rollback:** drop the pmp_20 functions + revision columns. **Verify:** must not silently invalidate member responses (UC-MENU-005) — invalidated responses reset to `draft` + the member is notified to re-confirm.
...
### MP-A-121 — Menu authoring/publish service + APIs — `PARTIAL` (publish + authoring + edit/revision writers shipped; builder UI remains)
...
- **Partial 1 shipped (ADO #84):** the **pure** `validateMenuCompleteness(menuDay, now): ValidationIssue[]` — the ADR-7-independent structural-completeness validator (`03`§5) — landed in `@mmp/shared/provider/menu-completeness.ts` (+ `isMenuPublishable`). It covers the rules verifiable on a `MenuDayDto`: non-empty menu; every _required_ component has a non-empty default item id + denormalized name; positive default/alternative quantities; units consistent within a component; `quantity_increment` customizations declare a finite group + option max; required customization groups well-formed (`min≥1`, ≥1 option, `min≤max`); `cutoffAt` in the future. 16 Vitest cases + the canonical `publishedMenuDay` fixture asserted publishable.
- **Partial 2 shipped (ADO #22, migration `pmp_18_menu_publish`):** the **fresh-publish writer** `publish_provider_menu_day(menu_day_id)` (SECURITY DEFINER) + `publishMenuDay` service + `POST /api/provider-menu-days/{id}/publish`. ADR-7-independent (publishing a draft has no responses). It owner-gates, draft-gates (idempotent on an already-published day; `PMNDR`→`menu_not_draft` for terminal states), enforces the **DB-context** completeness #84 left out (every default + active alternative references an **active** catalog item **owned by this provider** — no archived/cross-provider/dangling ref; `PMINC`→`menu_incomplete` with per-ref issues), flips the day **and** its weekly container to `published`, and fans out `provider_menu_published` to **active** customers (UC-NOTIFY-001) via `emit_provider_event`. The service reuses the #84 structural validator before the RPC. Backend-only DoD: 11 Vitest service cases + 3 integration/RLS specs (owner-publishes + notifies, customer `PMOWN`, archived-item `PMINC`) + mobile contract test; `seedMenuDay` extended with a `draft` status. New reason `menu_not_draft` (CONFLICT 409).
- **Partial 3 shipped (ADO #22, migration `pmp_20_menu_edit_revision`):** the **edit/revision writer** — `edit_provider_menu_day(menu_day_id, payload)` (SECURITY DEFINER) + `reviseMenuDay` service + `PUT /api/provider-menu-days/{id}`, plus the non-structural `update_provider_menu_day_note` + `updateMenuDayNote` + `PATCH /api/provider-menu-days/{id}`. Realises MP-A-012E (above) and the ADR-7 = REVISION rebuild: in-place tree rebuild when no response exists; else a rev N+1 (a fresh published day for the same date carrying the new tree) with existing responses carried forward + server-re-derived, selective per-component invalidation (a dropped/changed line → response reset to `draft` + member notified to re-confirm), the prior revision `archived` + `superseded_at` stamped, and the revision recorded in `provider_menu_revised` event fan-out. Added a revision dimension to `provider_menu_days` (`revision`, `supersedes_menu_day_id`, `superseded_at`; today/weekly reads serve only the live revision). The create-tree build loop was extracted into the shared `insert_provider_menu_component_tree` helper (create + edit reuse it; forward-fix rewire like pmp_16). New reason `menu_not_editable` (CONFLICT 409). Backend-only DoD (no RN screen — builder edit UI MP-B-030/MP-C-030 below): `EditMenuDayInput`/`UpdateMenuDayNoteInput` DTOs + `revision`/`supersedesMenuDayId`/`supersededAt` on `MenuDayDto` + `reviseMenuDay`/`updateMenuDayNote` on the shared `ProviderApiClient` (web + mobile client + both mocks) + mobile Jest routing test; Vitest service tests + a rolled-back MCP behaviour/RLS probe. Applied to cloud dev + types patched surgically.
- **Remaining:** the builder edit-affordance UI (web MP-B-030 + mobile MP-C-030) consuming `reviseMenuDay`/`updateMenuDayNote`, and member suggestions MP-A-131. Tracked under #22 (stays in Doing) + siblings #31/#36/#37.
...
### MP-B-050 — Provider preparation UI — `DONE` (ADO #26, PR pending)

- **Track:** B · **Checkpoint:** 5 · **Branch:** feature/provider-preparation-ui-mp-b-050.
...
- **Shipped:** the owner Preparation index (`/provider/preparation`) — lists each menu day's CURRENT batch (census summary + email status), newest first — and the detail page (`/provider/preparation/{batchId}`): cutoff census, aggregate cooking roster, per-member breakdown, aggregate + per-member CSV downloads, summary-email status + resend, and regenerate. Backed by a new request-scoped `listProviderBatches` (owner reads batches+menu_days directly under existing RLS — **no migration**) + `getProviderBatchForMenuDay`, which makes the previously-routeless `getPreparationBatch(menuDayId)` contract real (`GET /api/provider-menu-days/{menuDayId}/preparation-batch`) alongside `GET /api/providers/{id}/preparation-batches`. New shared `ProviderBatchSummaryDto` + `ProviderBatchDetailDto` (the batch read now carries `menuDate`/`cutoffAt`/`providerName` on the wire so web + mobile render the date) + `listBatches` on the `ProviderApiClient` seam (web/mobile mocks + mobile HTTP client). Shared `providerVariantSuffix`/`providerSpiceLabel`/`providerSaltLabel` label helpers extracted (summary-email renderer now reuses them). Tests: Vitest `listProviderBatches`/`getProviderBatchForMenuDay`; Playwright `provider-preparation-ui.spec.ts` (index→detail→roster, empty state, owner-only gate); paired mobile MP-C-050. The `@media print` page is MP-B-051; mobile share/export is MP-C-051.

### MP-B-051 — Print page UI — `DONE` (ADO #26, PR pending)

- **Track:** B · **Checkpoint:** 5 · **Branch:** feature/provider-print-share-mp-b-051.
