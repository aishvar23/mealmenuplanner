...
- **Partial shipped (ADO #84):** the **pure** `validateMenuCompleteness(menuDay, now): ValidationIssue[]` — the ADR-7-independent structural-completeness validator (`03`§5) — landed in `@mmp/shared/provider/menu-completeness.ts` (+ `isMenuPublishable`). It covers the rules verifiable on a `MenuDayDto`: non-empty menu; every _required_ component has a non-empty default item id + denormalized name; positive default/alternative quantities; units consistent within a component; `quantity_increment` customizations declare a finite group + option max; required customization groups well-formed (`min≥1`, ≥1 option, `min≤max`); `cutoffAt` in the future. The catalog item active/ownership + cross-provider + required-GROUP-coverage checks need DB context and stay in the (still ADR-7-blocked) publish RPC. Shared-only DoD (no RN screen; mobile contract green); 16 Vitest cases + the canonical `publishedMenuDay` fixture asserted publishable. Consumed by this task's publish RPC and MP-B-030/MP-C-030's "incomplete blocks publish" affordance once ADR-7 unblocks. The rest of MP-A-121 (create/publish RPC + edit policy) **stays BLOCKED on ADR-7 (#30)**.
