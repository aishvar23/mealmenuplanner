...
`family_size`, `adults_count`, `kids_count`, `diet_types` (BETA multi-select; the
loader reads the array and falls back to the legacy scalar `diet_type`),
`preferred_cuisines`,
...
| ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Violates diet type**                            | `dishes.diet_type` is not compatible with **any** of the household's selected diets (`household_preferences.diet_types`, BETA multi-select — a dish passes if it matches at least one), then tightened by any stricter member `user_food_preferences.diet_type` (applied as an extra AND, so a member restriction narrows but never widens the union). E.g. a `vegetarian`-only household excludes `non_vegetarian`; a `vegetarian` + `non_vegetarian` household accepts both; `vegan` excludes `vegetarian` dishes containing dairy/egg ingredients; `jain` excludes onion/garlic-containing dishes. |
