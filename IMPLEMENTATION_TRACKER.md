...
| P1    | Auth & household foundation | 3 / 8        | In progress |
...
like `P0-3`'s prod project). `P1-4` (`lib/auth` permission guards:
`getActiveMembership`/`requireActiveMember`/`requirePermission` over
`permissions.ts` + `guards.ts`) is now done and CI-green — and it regenerated
`lib/db/database.types.ts` from cloud dev via MCP, so the service layer finally
has real table/enum types. **Next is `P1-5`** (`household` service: create
household + owner membership with all `can_*` = true, in one transaction) — it
unblocks `P1-6`..`P1-8` (the household APIs, which consume the new guards) and
P2's onboarding-complete path. `P1-2` (email/password + magic-link) is an
...
- [x] **P1-4** `lib/auth` permission guards: active-membership check + `can_*` flag check, returning typed errors — _Split like `session`/`route-access`: **pure** `lib/auth/permissions.ts` (edge-safe, no `server-only`) holds the vocabulary — `PERMISSION_FLAGS` (the 8 `can_*` flags), `Permission`/`MembershipContext` types, `isMembershipActive()` (mirrors `is_active_member()` incl. real-time `expires_at > now()`, strict `>`), `hasPermission()`, and the row→context mappers. **Server-only** `lib/auth/guards.ts` holds the I/O guards: `getActiveMembership()` (the non-throwing primitive — requires a verified session via `requireAuthUser()`, then loads the caller's `status='active'` `household_members` row via the per-request RLS client and applies the expiry backstop itself; returns the context or `null` for "authenticated but not a member"), `requireActiveMember()` → `ForbiddenError` (design/03 §6 node E2), and `requirePermission(h, perm)` → `ForbiddenError` (E2/E3). Flags are the runtime source of truth (design/03 §4); RLS stays the independent backstop. Read endpoints that want a 404 over a 403 (existence-hiding, design/04 §2) use `getActiveMembership()` + translate `null`→`NotFoundError`; the returned context feeds P1-6's `currentUserPermissions`. **Side effect:** this is the first code to query a domain table, so `lib/db/database.types.ts` was regenerated from cloud dev (`dultruvperqxtqtbochp`) via the Supabase MCP `generate_typescript_types` (the `npm run db:types` script needs `--local`/Docker) — real types for all 19 tables, 17 enums, and the 2 RLS helper fns, replacing the empty stub. 21 new unit tests (42 total); lint · format · typecheck · test · build all green._
