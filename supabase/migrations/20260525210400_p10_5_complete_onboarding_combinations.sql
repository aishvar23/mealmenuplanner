-- P10-5: persist the redesigned preferred-dishes step's combination/build data on
-- completion. Adds a `p_combination_prefs jsonb` parameter to complete_onboarding:
--   • selectedCombinationIds — `combinations` mode: bump each active combo's
--     popularity_count (the ranking signal the picker + engine read).
--   • builtDishes — `build` mode: per main dish, upsert household_dish_preferences
--     (frequency tier), insert the "goes with" household_dish_accompaniments, and
--     bump the dish's popularity_count. The mains also arrive as liked_dishes via
--     p_food_preferences, so the engine's +10 liked bonus still fires.
--
-- The signature CHANGES (4 → 5 args). A `create or replace` with extra defaulted
-- args would OVERLOAD (leaving the 4-arg version callable and making a 4-arg call
-- ambiguous), so the old version is dropped first. The onboarding service is the
-- sole caller. Same security model as P2-6/P9: SECURITY DEFINER, search_path = '',
-- EXECUTE revoked from public/anon and granted to authenticated.
drop function if exists public.complete_onboarding(uuid, jsonb, jsonb, jsonb);
...
begin
  ...
  if p_combination_prefs is not null then
    ...
    -- `build` mode: per main dish, record frequency + accompaniments + popularity.
    for v_built in
      select value
    ...
    loop
      ...
      if v_dish_id is not null then
        insert into public.household_dish_preferences (
          household_id, dish_id, frequency
        )
        values (
          v_household_id,
          v_dish_id,
