-- P10-9 · Per-combination "how often" + "suitable for" in onboarding (Mode 1).
--
-- The "Select meal combinations" mode now mirrors the build-your-own per-dish
-- controls — a frequency tier and the meal slots the plate is "suitable for" —
-- minus "goes with" (a combination already fixes the dishes that go together).
-- The wizard sends these as `p_combination_prefs.selectedCombinations`, each entry
-- `{ combinationId, frequency, suitableFor }` (the pre-P10-9 `selectedCombinationIds`
-- id-only array is rehydrated to this shape in validate-completion.ts).
--
-- On completion, for each selected ACTIVE combination we (a) bump its popularity
-- (the ranking signal the picker + engine read, unchanged from P10-5) and (b) apply
-- the plate-level frequency + suitable slots to EVERY dish in the combination via
-- household_dish_preferences — exactly the rows the build mode writes, so the
-- recommendation engine (frequency tier + suitable_meal_slots hard filter,
-- design/05 § 4) honors a combination pick the same way it honors a built dish.
--
-- Same 5-arg signature as P10-8 (20260526120000) — a `create or replace` cleanly
-- swaps the body and preserves grants. The only change is the `combinations`-mode
-- section: a popularity-only UPDATE becomes a loop that also upserts the member
-- dishes' household_dish_preferences. Build mode is unchanged. Same security model:
-- SECURITY DEFINER, search_path = '', EXECUTE revoked from public/anon, granted to
-- authenticated.
